...
summary(factor(HousingPrice_Yunlin_S$TransactionType))


# 移除極值 TransLot TransArea --------------------------------------------------
nrow(subset(HousingPrice_Yunlin_S, HousingPrice_Yunlin_S$TransLot > quantile(HousingPrice_Yunlin_S$TransLot, probs = 0.75) + 5*var(HousingPrice_Yunlin_S$TransLot)^(1/2)))
HousingPrice_Yunlin_S <- subset(HousingPrice_Yunlin_S, TransLot < quantile(HousingPrice_Yunlin_S$TransLot, probs = 0.75) + 5*var(HousingPrice_Yunlin_S$TransLot)^(1/2))
nrow(subset(HousingPrice_Yunlin_S, HousingPrice_Yunlin_S$TransLot > quantile(HousingPrice_Yunlin_S$TransArea, probs = 0.75) + 5*var(HousingPrice_Yunlin_S$TransArea)^(1/2)))

# ------------------------------------------------------------------------------
HousingPrice_Yunlin_S$Age_sq <- HousingPrice_Yunlin_S$Age^2

# ---移除關係人交易
HousingPrice_Yunlin_S_norelated <- subset(HousingPrice_Yunlin_S, HousingPrice_Yunlin_S$RelatedPartyTrans != 1)
# ------------------------------------------------------------------------------
...
# Model 1
fit_log0 <- lm(log(TotalPrice) ~  factor(ZoneType) + TransactionType + factor(TransYear) + factor(TransQtr) + Age + Age_sq +
                BuildingType + TransArea + TransLot + Bedrooms + Livingrooms + Bathrooms +factor(ParkIncluded) + 
                DistanceToCBD + DistanceToHSR+ DistanceToIC +
                factor(TrainStationsNearby) + factor(BusStopNearby) +
                factor(ParkNearby) + factor(HospitalNearby) + factor(SchoolNearby),
              data = subset(HousingPrice_Yunlin_S_norelated, HousingPrice_Yunlin_S_norelated$TransYear >= 2015))
fit_log2 <- lm(log(TotalPrice) ~  factor(ZoneType) + TransactionType + factor(TransYear) * factor(TransQtr) + Age + Age_sq +
                BuildingType + TransArea + TransLot + Bedrooms + Livingrooms + Bathrooms +factor(ParkIncluded) + 
                DistanceToCBD + DistanceToHSR+ DistanceToIC +
                factor(TrainStationsNearby) + factor(BusStopNearby) +
                factor(ParkNearby) + factor(HospitalNearby) + factor(SchoolNearby),
              data = subset(HousingPrice_Yunlin_S_norelated, HousingPrice_Yunlin_S_norelated$TransYear >= 2015))
anova(fit_log, fit_log0)

res <- as.data.frame(fit_log$residuals)
colnames(res) <- "res"
HousingPrice_Yunlin_S_norelated <- cbind(HousingPrice_Yunlin_S_norelated, res)

summary(fit_log)
plot(fit_log$residuals)
plot(HousingPrice_Yunlin_S_norelated$res~(HousingPrice_Yunlin_S_norelated$DistanceToHSR))
...
HousingPrice_Yunlin_S_norelated$Time <- paste0(HousingPrice_Yunlin_S_norelated$TransYear, "_", HousingPrice_Yunlin_S_norelated$TransQtr)
fit_log <- lm(log(TotalPrice) ~  factor(ZoneType) + TransactionType + factor(Time) + Age + Age_sq +
                BuildingType + TransArea + TransLot + Bedrooms + Livingrooms + Bathrooms +factor(ParkIncluded) + 
                DistanceToCBD + DistanceToHSR+ DistanceToIC +
                factor(TrainStationsNearby) + factor(BusStopNearby) +
                factor(ParkNearby) + factor(HospitalNearby) + factor(SchoolNearby),
